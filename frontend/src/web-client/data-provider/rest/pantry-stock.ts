...
import { getPantryDashboard, type PantryDashboardResponse } from "@/modules/pantry/api";
...
export async function searchPantryStock(
  token: string,
  args: SearchArgs,
): Promise<SearchResult<StockRow>> {
  const dashboard = await getPantryDashboard(token);
  ...
  if (needle) {
    rows = rows.filter((row) => {
      const tokens = [
        ...
        String(row.minStock),
        row.isBelowMin ? "abaixo do mínimo" : "ok",
      ];
