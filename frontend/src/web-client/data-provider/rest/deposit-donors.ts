...
import { listDepositDonors } from "@/modules/deposit/api";
...
function matchesDonorCondition(row: ApiDepositDonor, condition: DomainCondition) {
...
async function listAllDonors(token: string, searchText?: string) {
  const limit = 200;
  let page = 1;
  const all: ApiDepositDonor[] = [];

  for (;;) {
    const res = await listDepositDonors(token, {
      page,
      limit,
...
export async function searchDepositDonors(
  token: string,
  args: SearchArgs,
): Promise<SearchResult<ApiDepositDonor>> {
  ...
  if (!needle && !hasLocalFilters && !args.all) {
    ...
    const res = await listDepositDonors(token, { page: pageIndex + 1, limit: pageSize });
    return {
      data: res.data ?? [],
