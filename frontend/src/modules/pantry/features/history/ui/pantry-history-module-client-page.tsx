...
import type { ApiPantryHistoryMovement } from "@/modules/pantry/api";
import {
  pantryHistoryModuleDefinition,
  type PantryHistoryQueryState,
...
type HistoryRow = ApiPantryHistoryMovement & {
  itemName: string;
  itemGroup: string;
};

type PantryHistoryFavoriteState = Pick<
  PantryHistoryQueryState,
  | "view"
  | "searchText"
  | "domain"
  | "groupBy"
...
const DEFAULT_CALENDAR_MODE: PantryHistoryQueryState["calendarMode"] = "month";
const DEFAULT_TIMELINE_FIELD = "movementDate";
const DEFAULT_TIMELINE_SORT: PantryHistoryQueryState["timelineSortDirection"] = "desc";
const CALENDAR_MODE_LABELS: Record<PantryHistoryQueryState["calendarMode"], string> = {
  day: "Dia",
  week: "Semana",
  month: "Mes",
  year: "Ano",
...
const historyGraphBuilder = getModuleGraphBuilderConfig(pantryHistoryModuleDefinition);

function normalizeView(value: string | null): PantryHistoryQueryState["view"] {
  switch (value) {
    case "kanban":
    case "timeline":
    case "calendar":
...
function normalizeCalendarMode(
  value: string | null,
): PantryHistoryQueryState["calendarMode"] {
  switch (value) {
    case "day":
    case "week":
    case "month":
...
const parseQueryState = (
  ...
  return {
    ...
    graph: historyGraphBuilder
      ...
      : pantryHistoryModuleDefinition.defaultQueryState.graph,
  };
};

const serializeQueryState = (state: PantryHistoryQueryState) => ({
...
function resolveKindLabel(kind: ApiPantryHistoryMovement["kind"]) {
...
function resolveGroupValue(row: ApiPantryHistoryMovement, field: string) {
  switch (field) {
    case "kind":
    ...
    case "expiryDate":
      return row.expiryDate ? formatDateOnlyPtBR(row.expiryDate) : "Sem validade";
    default:
      return String((row as Record<string, unknown>)[field] ?? "").trim() || "—";
  }
}

function resolveDate(row: ApiPantryHistoryMovement, field: string) {
  const raw = (row as Record<string, unknown>)[field];
  if (!raw) return null;
  return parseLocalDateOnly(String(raw));
}

export function PantryHistoryModuleClientPage() {
  const { token } = useAuth();
  ...
  const [rows, setRows] = React.useState<ApiPantryHistoryMovement[]>([]);
  const [loading, setLoading] = React.useState(true);
  const [error, setError] = React.useState<string | null>(null);
  const [pageCount, setPageCount] = React.useState(0);
  const [totalCount, setTotalCount] = React.useState(0);

  const [state, setState] = useModuleQueryState<PantryHistoryQueryState>({
  ...
  const valueSuggestions = React.useMemo(() => {
    ...
    rows.forEach((row) => {
      ...
      if (row.eventName?.trim()) events.add(row.eventName.trim());
    });
    ...
    return (fieldName: string) => {
      ...
      if (fieldName === "eventName") return eventList;
      return [];
    };
  }, [rows]);
  ...
  const loadRows = React.useCallback(async () => {
    ...
    try {
      const response = await dataProvider.search<ApiPantryHistoryMovement>(
        pantryHistoryModuleDefinition.queryAdapters.listDataProvider.model,
        {
  ...
  const queryFacets = React.useMemo(
    () =>
      buildQueryFacets({
        ...
        searchView: pantryHistoryModuleDefinition.searchConfig,
        onClearGroupBy: () =>
  ...
  const dateFields = React.useMemo(
    () =>
      pantryHistoryModuleDefinition.searchConfig.fields.filter(
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <Select
            ...
            onValueChange={(value) =>
              setState((previous) => ({
                ...previous,
                timelineSortDirection: value as PantryHistoryQueryState["timelineSortDirection"],
              }))
            }
          >
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <Select
            ...
            onValueChange={(calendarMode) =>
              setState((previous) => ({
                ...previous,
                calendarMode: calendarMode as PantryHistoryQueryState["calendarMode"],
              }))
            }
          >
  ...
  const analysisFacets = React.useMemo<SearchFacet[]>(() => {
    ...
    const defaultState = pantryHistoryModuleDefinition.defaultQueryState;
  ...
  const kanbanField = React.useMemo(() => {
    ...
    const kanbanView = pantryHistoryModuleDefinition.views.find(
  ...
  return (
    <EntityModuleShell<PantryHistoryQueryState, PantryHistoryFavoriteState>
      moduleDefinition={pantryHistoryModuleDefinition}
      state={state}
      ...
      onResetAnalysis={() =>
        setState((previous) => ({
          ...
          graph: pantryHistoryModuleDefinition.defaultQueryState.graph,
        }))
      }
    >
      {error ? (
          ...
          {state.view === "kanban" ? (
            ...
            <ListView<HistoryRow>
              ...
              resolveGroupLabel={(field, value) => {
                ...
                if (field === "eventName") return String(value ?? "").trim() || "Sem evento";
                if (field === "donorName") return String(value ?? "").trim() || "Sem doador";
                if (field === "quantity") {
