...
import {
  addPantryExitAttachment,
  addPantryExitComment,
  createPantrySector,
  deletePantryExitAttachment,
  deletePantryExitComment,
  listPantryEntries,
  listPantrySectors,
  type ApiPantryEntry,
  type ApiPantryExit,
...
import {
  ...
  type PantryExitDraft,
} from "@/modules/pantry/config/pantry-detail-layout-contract";
import { pantryExitDetailModuleDefinition } from "@/modules/pantry/config/pantry-module-contract";
import {
...
function draftFromExit(exit: ApiPantryExit): PantryExitDraft {
  return {
    entryId: exit.entryIds?.length ? String(exit.entryIds[0] ?? "") : "",
    sector: exit.sector ?? PANTRY_DEFAULT_SECTOR,
    quantity: Number(exit.quantity ?? 0),
    unit: exit.unit ?? PANTRY_DEFAULT_UNIT,
    exitDate: String(exit.exitDate ?? "").slice(0, 10),
    type: exit.type ?? "ATTENDED",
    eventName: exit.eventName ?? "",
    notes: exit.notes ?? "",
  };
}

function resolveRemaining(entry: ApiPantryEntry) {
...
export function PantryExitDetailEngineClientPage({
  mode,
}: {
  mode: Mode;
}) {
  ...
  const [exit, setExit] = React.useState<ApiPantryExit | null>(null);
  const [draft, setDraft] = React.useState<PantryExitDraft>({
    entryId: "",
  ...
  const usageItems = React.useMemo(
    () => buildUsageDocumentationItems("pantry-exits-detail", PANTRY_USAGE_TEXT.exits.sections),
    [],
  );

  useRegisterUsageDocumentation({
    title: PANTRY_USAGE_TEXT.exits.title,
    items: usageItems,
  });

  const persistExitDraft = React.useCallback(
    async (nextDraft: PantryExitDraft) => {
      if (!exit) {
        return nextDraft;
      }

      const updated = await dataProvider.update<ApiPantryExit>("pantry.exits", exit.id, {
        ...
        type: nextDraft.type,
        eventName: normalizeStringOrNull(nextDraft.eventName),
  ...
  const autoSave = useDetailAutoSaveController<PantryExitDraft>({
    draft,
    enabled: mode === "edit" && canUpdate && Boolean(exit),
    config: getModuleDetailEditingConfig(pantryExitDetailModuleDefinition),
  ...
  const loadExit = React.useCallback(async () => {
    ...
    setLoading(true);
    try {
      const loaded = await dataProvider.read<ApiPantryExit>("pantry.exits", id);
  ...
  const loadEntries = React.useCallback(async () => {
    ...
    try {
      const response = await listPantryEntries(token, { itemId: nextItemId, all: true });
  ...
  const createExit = React.useCallback(async () => {
    ...
    setBusy(true);
    try {
      await dataProvider.create<ApiPantryExit>("pantry.exits", {
        ...
        type: draft.type,
        eventName: draft.eventName.trim() ? draft.eventName.trim() : null,
  ...
  const persistMetadata = React.useCallback(
    async (patch: { tags?: string[]; internalNotes?: string | null }) => {
      ...
      const updated = await dataProvider.update<ApiPantryExit>("pantry.exits", id, patch);
      setExit(updated);
  ...
  const handleDelete = React.useCallback(async () => {
    ...
    setBusy(true);
    try {
      await dataProvider.delete("pantry.exits", exit.id);
  ...
  const entryLabelById = React.useMemo<Record<string, string>>(
    () => Object.fromEntries(entries.map((entry) => [entry.id, buildPantryEntryAuditLabel(entry)])),
    [entries],
  );

  const entryById = React.useMemo<Record<string, ApiPantryEntry>>(
  ...
  const submitComment = React.useCallback(async () => {
    ...
    try {
      const updated = await addPantryExitComment(token, id, {
  ...
  const deleteComment = React.useCallback(
    async (commentId: string) => {
      ...
      const updated = await deletePantryExitComment(token, id, commentId);
      setExit(updated);
    },
    [id, mode, token],
  );
  ...
  const uploadAttachment = React.useCallback(async () => {
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        const updated = await addPantryExitAttachment(token, id, {
  ...
  const deleteAttachment = React.useCallback(
    async (attachmentId: string) => {
      if (!token || mode !== "edit" || !id) return;
      const updated = await deletePantryExitAttachment(token, id, attachmentId);
      setExit(updated);
    },
    [id, mode, token],
  );

  const historyItems = React.useMemo<PantryExitDetailLayoutContext["historyItems"]>(
    () => {
      const items: PantryExitDetailLayoutContext["historyItems"] = [];
      if (exit) {
        items.push({
          id: `${exit.id}:created`,
          title: "Saída criada",
  ...
  const context = React.useMemo<PantryExitDetailLayoutContext>(
    () => ({
      mode,
      readOnly,
      saving,
  ...
  return (
    <>
      <DetailShellEngine<PantryExitDetailLayoutContext>
        ...
        breadcrumbItems={[
          ...
          ...(mode === "edit" && exit?.item?.name
            ? [{ label: exit.item.name, href: `${PANTRY_ROUTES.items}/${exit.itemId}` }]
...
export function PantryExitNewEngineClientPage() {
  return <PantryExitDetailEngineClientPage mode="create" />;
}

export function PantryExitEditEngineClientPage() {
  return <PantryExitDetailEngineClientPage mode="edit" />;
}
