...
import type { ApiPantryEntry, ApiPantryExit, ApiPantryHistoryMovement } from "@/modules/pantry/api";
import type { PantryItemDetailMode } from "@/modules/pantry/features/items/domain/pantry-item-detail.types";
import { resolveExitTypeLabel } from "@/modules/pantry/features/items/domain/pantry-item-detail.helpers";
import { PANTRY_DEFAULT_SECTOR } from "@/modules/pantry/shared/domain/pantry.constants";
...
export type PantryItemMovementsTabsProps = {
...
export function PantryItemMovementsTabs({
  mode,
  readOnly,
  movementsLoading,
  entryRows,
  ...
  onOpenEntry,
  onOpenExit,
}: PantryItemMovementsTabsProps) {
  const entryColumns = React.useMemo<ColumnDef<ApiPantryEntry>[]>(
    () => [
      {
        id: "entryDate",
        header: "Data",
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="truncate text-sm">
            {row.original.donor?.name ?? "Sem doador"}
          </div>
        ),
      },
      {
  ...
  const exitColumns = React.useMemo<ColumnDef<ApiPantryExit>[]>(
    () => [
      {
        id: "exitDate",
        header: "Data",
  ...
  const historyColumns = React.useMemo<ColumnDef<ApiPantryHistoryMovement>[]>(
    () => [
      {
        id: "kind",
        header: "Movimento",
    ...
    <div className="pt-3">
      <Tabs defaultValue="entries">
        ...
        <TabsContent value="exits" className="mt-3 space-y-3">
          {mode !== "edit" ? (
            <div className="rounded-md border border-dashed border-border/60 p-4 text-center text-sm text-muted-foreground">
              Salve o alimento para registrar saídas.
            </div>
          ) : (
            <>
              <div className="flex items-center justify-end">
        ...
        <TabsContent value="history" className="mt-3 space-y-3">
          {mode !== "edit" ? (
            <div className="rounded-md border border-dashed border-border/60 p-4 text-center text-sm text-muted-foreground">
              Salve o alimento para ver o histórico.
            </div>
          ) : movementsLoading ? (
            <div className="text-sm text-muted-foreground">Carregando…</div>
          ) : historyRows.length ? (
            <ListView<ApiPantryHistoryMovement>
              data={historyRows}
