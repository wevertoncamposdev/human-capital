...
import type { DepositDashboardResponse } from "@/modules/deposit/api";
import {
  depositStockModuleDefinition,
  type DepositStockQueryState,
...
type DepositStockFavoriteState = Pick<
  DepositStockQueryState,
  | "view"
  | "searchText"
  | "domain"
  | "groupBy"
...
const DEFAULT_CALENDAR_MODE: DepositStockQueryState["calendarMode"] = "month";
const DEFAULT_TIMELINE_FIELD = "nextExpiryDate";
const DEFAULT_TIMELINE_SORT: DepositStockQueryState["timelineSortDirection"] = "desc";
const CALENDAR_MODE_LABELS: Record<DepositStockQueryState["calendarMode"], string> = {
  day: "Dia",
  week: "Semana",
  month: "Mês",
  year: "Ano",
...
const stockGraphBuilder = getModuleGraphBuilderConfig(depositStockModuleDefinition);

function normalizeView(value: string | null): DepositStockQueryState["view"] {
  switch (value) {
    case "kanban":
    case "timeline":
    case "calendar":
...
function normalizeCalendarMode(
  value: string | null,
): DepositStockQueryState["calendarMode"] {
  switch (value) {
    case "day":
    case "week":
    case "month":
...
const parseQueryState = (
  ...
  return {
    ...
    graph: stockGraphBuilder
      ...
      : depositStockModuleDefinition.defaultQueryState.graph,
  };
};

const serializeQueryState = (state: DepositStockQueryState) => ({
...
export function DepositStockModuleClientPage() {
  const { token } = useAuth();
  ...
  const createLabel = React.useMemo(
    () => getModuleActionLabel(depositStockModuleDefinition, "create", "Novo"),
    [],
  );
  ...
  const [state, setState] = useModuleQueryState<DepositStockQueryState>({
  ...
  const loadRows = React.useCallback(async () => {
    ...
    try {
      const response = await dataProvider.search<StockRow>(
        depositStockModuleDefinition.queryAdapters.listDataProvider.model,
        {
  ...
  const queryFacets = React.useMemo(
    () =>
      buildQueryFacets({
        ...
        searchView: depositStockModuleDefinition.searchConfig,
        onClearGroupBy: () =>
  ...
  const dateFields = React.useMemo(
    () =>
      depositStockModuleDefinition.searchConfig.fields.filter(
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <Select
            ...
            onValueChange={(value) =>
              setState((previous) => ({
                ...previous,
                timelineSortDirection: value as DepositStockQueryState["timelineSortDirection"],
              }))
            }
          >
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <Select
            ...
            onValueChange={(calendarMode) =>
              setState((previous) => ({
                ...previous,
                calendarMode: calendarMode as DepositStockQueryState["calendarMode"],
              }))
            }
          >
  ...
  const analysisFacets = React.useMemo<SearchFacet[]>(() => {
    ...
    const defaultState = depositStockModuleDefinition.defaultQueryState;
  ...
  const kanbanField = React.useMemo(() => {
    ...
    const kanbanView = depositStockModuleDefinition.views.find(
  ...
  return (
    <EntityModuleShell<DepositStockQueryState, DepositStockFavoriteState>
      moduleDefinition={depositStockModuleDefinition}
      state={state}
      ...
      onResetAnalysis={() =>
        setState((previous) => ({
          ...
          graph: depositStockModuleDefinition.defaultQueryState.graph,
        }))
      }
      primaryActionSlot={
        ...
        <>
          {state.view === "kanban" ? (
            ...
            <CalendarView<StockRow>
              ...
              emptyState="Nenhuma data limite neste dia."
              renderItem={(row) => (
                <div
                  className="rounded-md border border-border/60 bg-background px-3 py-2 hover:bg-muted/20"
                  role="button"
            ...
            <ListView<StockRow>
              ...
              resolveGroupLabel={(field, value) => {
                ...
                if (field === "validityStatus") return String(value ?? "").trim() || "Sem data limite";
                if (field === "isBelowMin") {
