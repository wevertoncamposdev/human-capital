...
import { toInt } from "@/modules/deposit/features/items/domain/deposit-item-detail.helpers";
import {
  DEPOSIT_ITEM_GROUP,
  DEPOSIT_ITEM_UNIT,
} from "@/modules/deposit/shared/domain/deposit.constants";

export type DepositItemDetailFormProps = {
...
export function DepositItemDetailForm({
  mode,
  readOnly,
  draft,
  setDraft,
  ...
  onOpenNewSector,
  inputLineClassName,
}: DepositItemDetailFormProps) {
  const unitOptions = React.useMemo(() => {
    const base = DEPOSIT_ITEM_UNIT.map((opt) => ({
  ...
  const groupOptions = React.useMemo(() => {
    const base = DEPOSIT_ITEM_GROUP.map((opt) => ({
      label: String(opt.label),
      value: String(opt.value),
    }));
