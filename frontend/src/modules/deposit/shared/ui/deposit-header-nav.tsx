...
export function DepositHeaderNav({ className }: DepositHeaderNavProps) {
  const pathname = usePathname();
  const router = useRouter();
  const tenantSlug = getTenantSlugFromPath(pathname);

  const stockHref = withTenantPath(DEPOSIT_ROUTES.stock, tenantSlug);
  const donorsHref = withTenantPath(DEPOSIT_ROUTES.donors, tenantSlug);
  const historyHref = withTenantPath(DEPOSIT_ROUTES.history, tenantSlug);
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          aria-label="Menu do depósito"
          title="Menu do depósito"
        >
