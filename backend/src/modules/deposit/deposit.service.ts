...
const validateExitBusinessRules = (params: {
  type?: string | null;
  ...
  const type = params.type ?? 'FINAL_REMOVAL';

  if ((type === 'LOAN' || type === 'FINAL_REMOVAL' || type === 'TRANSFER') && !params.destinationName) {
    throw new BadRequestException('Informe o destino da saida.');
  }

  if (type === 'LOAN' && !params.expectedReturnAt) {
    throw new BadRequestException('Informe a data prevista de retorno para emprestimos.');
  }

  if (type !== 'LOAN' && params.expectedReturnAt) {
    throw new BadRequestException('Retorno previsto so pode ser usado em emprestimos.');
  }

  if (params.returnedQuantity !== null && params.returnedQuantity !== undefined && params.returnedQuantity < 0) {
    throw new BadRequestException('Quantidade retornada invalida.');
  }

  if ((params.returnedAt || (params.returnedQuantity ?? 0) > 0) && type !== 'LOAN') {
    throw new BadRequestException('Dados de retorno so podem ser usados em emprestimos.');
  }
};
...
export class DepositService {
  ...
  private serializeCommentAuthor(
    ...
    return {
      ...
      name: authorUser?.name || authorUser?.email || 'Usuario',
  ...
  private serializeExit(
    exit: {
      ...
      expectedReturnAt: Date | null;
      returnedAt: Date | null;
      returnedQuantity: Prisma.Decimal | number | null;
    ...
    return {
      ...
      destinationType: exit.destinationType ?? null,
      expectedReturnAt: exit.expectedReturnAt ?? null,
      returnedAt: exit.returnedAt ?? null,
      returnedQuantity: exit.returnedQuantity === null || exit.returnedQuantity === undefined ? null : toNumber(exit.returnedQuantity),
      returnNotes: exit.returnNotes ?? null,
      notes: exit.notes,
  ...
  async listHistory(
    ...
    const destinationName = normalizeString(params?.destinationName);
    ...
    if (destinationName) {
      const like = `%${destinationName}%`;
      exitWhere.push(Prisma.sql`x.destinationName LIKE ${like}`);
    }
    ...
    const entrySelect = Prisma.sql`
      ...
      FROM \`DepositEntry\` e
      JOIN \`DepositItem\` i ON i.id = e.itemId
      LEFT JOIN \`DepositDonor\` d ON d.id = e.donorId
    ...
    const exitSelect = Prisma.sql`
      SELECT
        ...
        NULL AS donorName,
        x.type AS exitType,
        x.destinationName AS destinationName
      FROM \`DepositExit\` x
      JOIN \`DepositItem\` i ON i.id = x.itemId
    ...
    const [createdByMap, removedByMap] = await Promise.all([
      this.resolveAuditUserLabels(tenantId, 'DepositEntry', entryIds),
      this.resolveAuditUserLabels(tenantId, 'DepositExit', exitIds),
    ]);
  ...
  private async syncSectorsFromData(tenantId: string) {
    const [entrySectors, exitSectors, items] = await this.prisma.$transaction([
      this.prisma.depositEntry.groupBy({
        by: ['sector'],
        where: { tenantId },
        orderBy: { sector: 'asc' },
      }),
      this.prisma.depositExit.groupBy({
        by: ['sector'],
        where: { tenantId },
        orderBy: { sector: 'asc' },
      }),
      this.prisma.depositItem.findMany({
        where: { tenantId },
    ...
    await this.prisma.depositSector.createMany({
  ...
  private async ensureSector(
    ...
    await client.depositSector.upsert({
  ...
  async listSectors(tenantId: string) {
    ...
    const sectors = await this.prisma.depositSector.findMany({
      where: { tenantId },
      orderBy: { name: 'asc' },
    });
  ...
  async createSector(tenantId: string, dto: { name: string }) {
    ...
    const sector = await this.prisma.depositSector.upsert({
      where: { tenantId_name: { tenantId, name } },
      create: { tenantId, name },
      update: {},
    });
  ...
  private async sumExitAllocationsByEntry(
    ...
    const groups = await client.depositExitAllocation.groupBy({
      by: ['entryId'],
      where: {
        tenantId,
        entryId: { in: entryIds },
  ...
  async listItems(tenantId: string, query: ListDepositItemsQueryDto) {
    const { page, limit, skip, take, isAll } = resolvePagination({
      page: query.page,
      limit: query.limit,
      all: query.all,
    ...
    const where: Prisma.DepositItemWhereInput = {
      tenantId,
      ...(search
        ? {
            OR: [
    ...
    const [items, total] = await this.prisma.$transaction([
      ...
      this.prisma.depositItem.count({ where }),
    ]);
  ...
  async getEntry(tenantId: string, id: string) {
    const entry = await this.prisma.depositEntry.findFirst({
      where: { id, tenantId },
      include: {
    ...
    const createdByMap = await this.resolveAuditUserLabels(tenantId, 'DepositEntry', [id]);
  ...
  async getItem(tenantId: string, id: string) {
    const item = await this.prisma.depositItem.findFirst({
      where: { id, tenantId },
      include: {
        comments: {
          orderBy: { createdAt: 'desc' },
  ...
  async listItemAuditLogs(
    ...
    const [entryRows, exitRows] = await this.prisma.$transaction([
      ...
      this.prisma.depositExit.findMany({
        where: { tenantId, itemId },
        select: { id: true },
      }),
    ]);
  ...
  async createItem(tenantId: string, dto: CreateDepositItemDto) {
    const defaultSector = await this.ensureSector(tenantId, dto.defaultSector ?? 'Geral');
    const item = await this.prisma.depositItem.create({
      data: {
        tenantId,
        name: dto.name,
  ...
  async updateItem(tenantId: string, id: string, dto: UpdateDepositItemDto) {
    ...
    const item = await this.prisma.depositItem.update({
      where: { id },
      data: {
        name: dto.name,
  ...
  async deleteItem(tenantId: string, id: string) {
    ...
    await this.prisma.$transaction([
      this.prisma.depositItemComment.deleteMany({ where: { tenantId, itemId: id } }),
      this.prisma.depositItemAttachment.deleteMany({ where: { tenantId, itemId: id } }),
      this.prisma.depositItem.delete({ where: { id } }),
    ]);
    return { ok: true };
  }
  ...
  async listDonors(
    ...
    const where: Prisma.DepositDonorWhereInput = {
      tenantId,
      ...(search
        ? {
            OR: [
    ...
    const [data, total] = await this.prisma.$transaction([
      this.prisma.depositDonor.findMany({
        where,
        orderBy: { name: 'asc' },
        skip: isAll ? undefined : skip,
      ...
      this.prisma.depositDonor.count({ where }),
    ]);
  ...
  async createDonor(tenantId: string, dto: CreateDepositDonorDto) {
    const donor = await this.prisma.depositDonor.create({
      data: {
        tenantId,
        name: dto.name,
  ...
  async getDonor(tenantId: string, id: string) {
    const donor = await this.prisma.depositDonor.findFirst({
      where: { id, tenantId },
      include: {
        comments: {
          orderBy: { createdAt: 'desc' },
  ...
  async listDonorAuditLogs(
    ...
    const entryRows = await this.prisma.depositEntry.findMany({
  ...
  async listEntryAuditLogs(
    ...
    const allocationRows = await this.prisma.depositExitAllocation.findMany({
  ...
  async updateDonor(tenantId: string, id: string, dto: UpdateDepositDonorDto) {
    const donor = await this.prisma.depositDonor.findFirst({
      where: { id, tenantId },
    });
    if (!donor) {
    ...
    const updated = await this.prisma.depositDonor.update({
      where: { id },
      data: {
        name: dto.name,
        type: dto.type,
  ...
  async deleteDonor(tenantId: string, id: string) {
    ...
    await this.prisma.$transaction([
      this.prisma.depositDonorComment.deleteMany({ where: { tenantId, donorId: id } }),
      this.prisma.depositDonorAttachment.deleteMany({ where: { tenantId, donorId: id } }),
      this.prisma.depositDonor.delete({ where: { id } }),
    ]);
    return { ok: true };
  }
  ...
  async listEntries(
    ...
    const where: Prisma.DepositEntryWhereInput = {
      tenantId,
    ...
    const [data, total] = await this.prisma.$transaction([
      ...
      this.prisma.depositEntry.count({ where }),
    ]);
  ...
  async createEntry(tenantId: string, dto: CreateDepositEntryDto) {
    ...
    const created = await this.prisma.depositEntry.create({
      data: {
        tenantId,
        itemId: dto.itemId,
  ...
  async updateEntry(tenantId: string, id: string, dto: UpdateDepositEntryDto) {
    const entry = await this.prisma.depositEntry.findFirst({
      where: { id, tenantId },
    });
    if (!entry) {
    ...
    const allocations = await this.prisma.depositExitAllocation.count({
    ...
    const updated = await this.prisma.depositEntry.update({
      where: { id },
      data: {
        itemId: dto.itemId,
  ...
  async deleteEntry(tenantId: string, id: string) {
    ...
    await this.prisma.$transaction([
      this.prisma.depositEntryComment.deleteMany({ where: { tenantId, entryId: id } }),
      this.prisma.depositEntryAttachment.deleteMany({ where: { tenantId, entryId: id } }),
      this.prisma.depositEntry.delete({ where: { id } }),
    ]);
    return { ok: true };
  }
  ...
  async listExits(
    ...
    const where: Prisma.DepositExitWhereInput = {
      tenantId,
      ...(params?.itemId ? { itemId: params.itemId } : {}),
    ...
    const [data, total] = await this.prisma.$transaction([
      ...
      this.prisma.depositExit.count({ where }),
    ]);
  ...
  async getExit(tenantId: string, id: string) {
    const exit = await this.prisma.depositExit.findFirst({
      where: { id, tenantId },
      include: {
        item: { select: { id: true, name: true, unit: true, group: true } },
        allocations: {
    ...
    const removedByMap = await this.resolveAuditUserLabels(tenantId, 'DepositExit', [id]);
  ...
  private async resolveOpenEntriesForExitAllocation(params: {
    ...
    const entries = await client.depositEntry.findMany({
      where: {
  ...
  async createExit(tenantId: string, dto: CreateDepositExitDto) {
    await this.assertItem(tenantId, dto.itemId);

    const normalizedSector = normalizeSector(dto.sector);
    const exitDate = parseDateInput(dto.exitDate) ?? new Date();
    const expectedReturnAt = parseDateInput(dto.expectedReturnAt) ?? null;
    const returnedAt = parseDateInput(dto.returnedAt) ?? null;
    const destinationName = normalizeString(dto.destinationName);
    const destinationType = normalizeString(dto.destinationType);
    const returnNotes = normalizeString(dto.returnNotes);
    const returnedQuantity =
      dto.returnedQuantity === undefined || dto.returnedQuantity === null
        ? null
        : Number(dto.returnedQuantity);

    validateExitBusinessRules({
      type: dto.type,
      destinationName,
      expectedReturnAt,
      returnedAt,
    ...
    const createdExitId = await this.prisma.$transaction(async (trx) => {
      ...
      const exit = await trx.depositExit.create({
        data: {
          tenantId,
          itemId: dto.itemId,
          sector: normalizedSector,
      ...
      if (dto.entryId) {
        const entry = await trx.depositEntry.findFirst({
          where: {
            id: dto.entryId,
            tenantId,
            itemId: dto.itemId,
        ...
        if (!entry) {
          throw new BadRequestException(
            'Entrada nao encontrada para esta saida.',
          );
        }
        ...
        if (dto.quantity > remaining) {
          throw new BadRequestException(
            `Saldo insuficiente na entrada selecionada (disponivel: ${remaining}).`,
          );
        }

        await trx.depositExitAllocation.create({
          data: {
            tenantId,
            exitId: exit.id,
            entryId: entry.id,
      ...
      if (allocations.length) {
        await trx.depositExitAllocation.createMany({
  ...
  async updateExit(tenantId: string, id: string, dto: UpdateDepositExitDto) {
    const exit = await this.prisma.depositExit.findFirst({
      where: { id, tenantId },
    });
    if (!exit) {
    ...
    const nextType = dto.type ?? exit.type;
    const destinationName =
      dto.destinationName !== undefined ? normalizeString(dto.destinationName) : exit.destinationName;
    const destinationType =
      dto.destinationType !== undefined ? normalizeString(dto.destinationType) : exit.destinationType;
    const expectedReturnAt =
      dto.expectedReturnAt !== undefined
        ? parseDateInput(dto.expectedReturnAt) ?? null
        : exit.expectedReturnAt ?? null;
    const returnedAt =
      dto.returnedAt !== undefined ? parseDateInput(dto.returnedAt) ?? null : exit.returnedAt ?? null;
    const returnedQuantity =
      dto.returnedQuantity !== undefined
        ? dto.returnedQuantity === null
          ? null
          : Number(dto.returnedQuantity)
        : exit.returnedQuantity === null || exit.returnedQuantity === undefined
          ? null
          : toNumber(exit.returnedQuantity);
    const returnNotes =
      dto.returnNotes !== undefined ? normalizeString(dto.returnNotes) : exit.returnNotes;
    ...
    const updatedExitId = await this.prisma.$transaction(async (trx) => {
      ...
      await trx.depositExitAllocation.deleteMany({
        where: { tenantId, exitId: id },
      });

      const updated = await trx.depositExit.update({
        where: { id },
        data: {
          itemId: dto.itemId,
  ...
  async deleteExit(tenantId: string, id: string) {
    ...
    await this.prisma.$transaction([
      this.prisma.depositExitComment.deleteMany({ where: { tenantId, exitId: id } }),
      this.prisma.depositExitAttachment.deleteMany({ where: { tenantId, exitId: id } }),
      this.prisma.depositExitAllocation.deleteMany({ where: { tenantId, exitId: id } }),
      this.prisma.depositExit.delete({ where: { id } }),
    ]);
    return { ok: true };
  }
  ...
  async addItemComment(
    tenantId: string,
    itemId: string,
    dto: CreateDepositItemCommentDto,
    actorUserId?: string,
  ) {
    await this.getItem(tenantId, itemId);
    await this.prisma.depositItemComment.create({
      data: {
        tenantId,
        itemId,
  ...
  async deleteItemComment(tenantId: string, itemId: string, commentId: string) {
    await this.getItem(tenantId, itemId);
    await this.prisma.depositItemComment.deleteMany({ where: { tenantId, itemId, id: commentId } });
    return this.getItem(tenantId, itemId);
  }
  ...
  async addItemAttachment(
    tenantId: string,
    itemId: string,
    dto: CreateDepositItemAttachmentDto,
    actorUserId?: string,
  ) {
    await this.getItem(tenantId, itemId);
    await this.prisma.depositItemAttachment.create({
      data: {
        tenantId,
        itemId,
  ...
  async deleteItemAttachment(tenantId: string, itemId: string, attachmentId: string) {
    await this.getItem(tenantId, itemId);
    await this.prisma.depositItemAttachment.deleteMany({
  ...
  async addDonorComment(
    tenantId: string,
    donorId: string,
    dto: CreateDepositDonorCommentDto,
    actorUserId?: string,
  ) {
    await this.getDonor(tenantId, donorId);
    await this.prisma.depositDonorComment.create({
      data: {
        tenantId,
        donorId,
        authorUserId: actorUserId ?? null,
  ...
  async deleteDonorComment(tenantId: string, donorId: string, commentId: string) {
    await this.getDonor(tenantId, donorId);
    await this.prisma.depositDonorComment.deleteMany({ where: { tenantId, donorId, id: commentId } });
    return this.getDonor(tenantId, donorId);
  }
  ...
  async addDonorAttachment(
    tenantId: string,
    donorId: string,
    dto: CreateDepositDonorAttachmentDto,
    actorUserId?: string,
  ) {
    await this.getDonor(tenantId, donorId);
    await this.prisma.depositDonorAttachment.create({
      data: {
        tenantId,
        donorId,
        uploadedByUserId: actorUserId ?? null,
  ...
  async deleteDonorAttachment(tenantId: string, donorId: string, attachmentId: string) {
    await this.getDonor(tenantId, donorId);
    await this.prisma.depositDonorAttachment.deleteMany({
  ...
  async addEntryComment(
    tenantId: string,
    entryId: string,
    dto: CreateDepositEntryCommentDto,
    actorUserId?: string,
  ) {
    await this.getEntry(tenantId, entryId);
    await this.prisma.depositEntryComment.create({
      data: {
        tenantId,
        entryId,
        authorUserId: actorUserId ?? null,
  ...
  async deleteEntryComment(tenantId: string, entryId: string, commentId: string) {
    await this.getEntry(tenantId, entryId);
    await this.prisma.depositEntryComment.deleteMany({ where: { tenantId, entryId, id: commentId } });
    return this.getEntry(tenantId, entryId);
  }
  ...
  async addEntryAttachment(
    tenantId: string,
    entryId: string,
    dto: CreateDepositEntryAttachmentDto,
    actorUserId?: string,
  ) {
    await this.getEntry(tenantId, entryId);
    await this.prisma.depositEntryAttachment.create({
      data: {
        tenantId,
        entryId,
        uploadedByUserId: actorUserId ?? null,
  ...
  async deleteEntryAttachment(tenantId: string, entryId: string, attachmentId: string) {
    await this.getEntry(tenantId, entryId);
    await this.prisma.depositEntryAttachment.deleteMany({
  ...
  async addExitComment(
    tenantId: string,
    exitId: string,
    dto: CreateDepositExitCommentDto,
    actorUserId?: string,
  ) {
    await this.getExit(tenantId, exitId);
    await this.prisma.depositExitComment.create({
      data: {
        tenantId,
        exitId,
        authorUserId: actorUserId ?? null,
  ...
  async deleteExitComment(tenantId: string, exitId: string, commentId: string) {
    await this.getExit(tenantId, exitId);
    await this.prisma.depositExitComment.deleteMany({ where: { tenantId, exitId, id: commentId } });
    return this.getExit(tenantId, exitId);
  }
  ...
  async addExitAttachment(
    tenantId: string,
    exitId: string,
    dto: CreateDepositExitAttachmentDto,
    actorUserId?: string,
  ) {
    await this.getExit(tenantId, exitId);
    await this.prisma.depositExitAttachment.create({
      data: {
        tenantId,
        exitId,
        uploadedByUserId: actorUserId ?? null,
  ...
  async deleteExitAttachment(tenantId: string, exitId: string, attachmentId: string) {
    await this.getExit(tenantId, exitId);
    await this.prisma.depositExitAttachment.deleteMany({
  ...
  async getSummary(tenantId: string) {
    const [items, donors, entryTotals, exitTotals] = await this.prisma.$transaction([
      ...
      this.prisma.depositDonor.count({ where: { tenantId } }),
      this.prisma.depositEntry.aggregate({
        where: { tenantId },
        _sum: { quantity: true },
      }),
      this.prisma.depositExit.aggregate({
        where: { tenantId },
        _sum: { quantity: true },
      }),
    ]);
  ...
  async getDashboard(tenantId: string) {
    const items = await this.prisma.depositItem.findMany({
      where: { tenantId },
      orderBy: { name: 'asc' },
    });
  ...
  async getConsumption(
    ...
    const destinationName = normalizeString(query.destinationName);
    ...
    const exits = await this.prisma.depositExit.findMany({
      where: {
        tenantId,
        exitDate: { gte: fromDate, lte: toDate },
        ...(query.itemId ? { itemId: query.itemId } : {}),
        ...(query.type ? { type: query.type } : { type: { not: 'ADJUSTMENT' } }),
        ...(destinationName ? { destinationName: { contains: destinationName } } : {}),
  ...
  async getUsage(
    ...
    const grouped = await this.prisma.depositExit.groupBy({
      by: ['type'],
      where: {
        tenantId,
        exitDate: { gte: fromDate, lte: toDate },
  ...
  async reverseEntry(
    ...
    const reversed = await this.prisma.depositExit.findFirst({
    ...
    const allocated = await this.prisma.depositExitAllocation.aggregate({
      where: { tenantId, entryId },
      _sum: { quantity: true },
    });
  ...
  async reverseExit(
    ...
    const existing = await this.prisma.depositEntry.findFirst({
    ...
    return this.prisma.$transaction(async (trx) => {
      const allocations = await trx.depositExitAllocation.findMany({
  ...
  private async enrichItems(tenantId: string, items: Prisma.DepositItemGetPayload<{}>[]) {
